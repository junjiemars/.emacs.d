...
Suppport any *Shell*, copy environment variables, and on any platforms.
...
If GNU's =find= has already in =PATH= so use it on Windows, and let Windows stupid =find= program gone.

Now, on Windows you can use =find-dired= or =find-name-dired=.
