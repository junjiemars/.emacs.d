...
It's works on Emacs version 22.1.1+ , provides more reasonable
style to organize Elisp code. You can run it on any platform that Emacs
supported with any mode: GUI or Terminal, even you can run multiple 
Emacs instances with different version on the same machine 
