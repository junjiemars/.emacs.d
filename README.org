...
=(emacs-home* "private/self-env-spec.el")= and for GUI mode only.
...
- =:shell-file-name= where the shell located
...
If GNU's =find= has already in =PATH= so use it on Windows, and let Windows stupid =find= program go away.
...
On Windows, there are no builtin =zip/unzip= program, but Emacs has a builtin =minizip= so We can 
zip, unzip or view the content a zip archive.

[[https://github.com/junjiemars/images/blob/master/.emacs.d/dired-do-compress-to-on-windows.png][screenshot: dired-do-compress-to]]

For some ancient Emacs such as below version 24, @@html:<kbd>@@! zip x.zip ?@@html:</kbd>@@ to zip.

[[https://github.com/junjiemars/images/blob/master/.emacs.d/dired-compress-file-suffixes-version-24lt.png][screenshot: dired-compress-file-suffixes]]
