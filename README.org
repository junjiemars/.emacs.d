...
The /user defined/ package spec default locate in =(emacs-home* "private/self-package-spec.el")=
...
- =:compile= when installed packages, a list of files that should be compiled only or be loaded after be compiled

You can use any =Elisp= functions after aboved keywords.
- =version-supported-p= macro checking the version of current Emacs at compile time
...
For Windows, there are no built-in =ls= program, but you can install GNU's =ls= via [[https://git-scm.com/downloads][Git-Bash]]. 
...
And more [[https://www.7-zip.org/download.html][7-Zip standalone command version also be supported]].

For some ancient Emacs24-, @@html:<kbd>@@! zip x.zip ?@@html:</kbd>@@ to zip.


/screenshots/:
- [[https://github.com/junjiemars/images/blob/master/.emacs.d/dired-do-compress-to-on-windows.png][dired-do-compress-to]]
- [[https://github.com/junjiemars/images/blob/master/.emacs.d/dired-view-file-on-windows.png][dired-view-file]]
...
Prefer =etags= program, because it works well on varied platforms. 
In =(emacs-home* "config/tags.el")=, there are some handy functions to create the tags for
=Elisp= or =C= source code, such as =make-emacs-source-tags=, =make-system-c-tags=, 
and =mount-tags=.
