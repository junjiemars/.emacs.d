...
=isearch-forward=, =isearch-forward-regexp= and
=isearch-forward-symbol-at-point=. =isearch-backward= and
=isearch-backward-regexp= has the same situation. Now, in More
Reasonable Emacs those functionalites uniformed to two functions:
=isearch-forward*= and =isearch-backward*=.

- @@html:<kbd>@@C-s@@html:</kbd>@@: do incremental search the symbol
  at point by default. If there are no symbol at point do incremental
  search regexp. If @@html:<kbd>@@C-u C-s@@html:</kbd>@@ do
  incremental search regexp.
- @@html:<kbd>@@C-r@@html:</kbd>@@: do incremental search symbol or
  regexp liked the above.
