...
(ert-deftest %d:fn:push! ()
  (should (equal '(a) (let ((s nil)) (push! 'a s))))
  (should (equal '(a b c) (let ((s '(b c))) (push! 'a s))))
  (should (equal '(a b c) (let ((s '(b c)))
                            (push! 'b s delq)
                            (push! 'a s delq))))
  (should (equal (list (list "a" "b") (list "c" "d"))
                 (let ((xs (list (list "c" "d"))))
                   (push! (list "c" "d") xs delete)
                   (push! (list "a" "b") xs delete)))))
