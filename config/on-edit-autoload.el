                            ...
                            (let ((m (funcall rproc ri)))
                              (cond ((and (consp m) (char= c (car m)))
                                    ...
                                    ((and (char= (caar ss)
                                     ...
                                     (throw 'q nil)))))
                           ((and l (not ss))
                            (let ((n (funcall lproc (1+ li)))
                                  (m (funcall rproc ri s)))
                              (cond ((and (not (consp n)) (consp m))
                                     (setq ri (cdr m))
                                     (throw 'q nil))
                                    ((consp m)
                                     (let ((m1 (funcall rproc ri)))
                                       (cond ((and (consp m1)
                                                   (char= s (car m1)))
                                              (setq ri (cdr m1))
                                              (throw 'q nil)))))
                                    (t (setq ss (cons (cons s li) ss))))))
