		 ...
		 (concat "@echo off\n"
						 ...
						 (cond ((string= "minizip" zip)
										(concat "\nREM parsing command line arguments\n\n"
														...
														"\n"
														":getopt\n"
														"if \"%1\"==\"-0\" set _LEVEL=%1 & shift & goto :getopt\n"
														"if \"%1\"==\"-1\" set _LEVEL=%1 & shift & goto :getopt\n"
														"if \"%1\"==\"-2\" set _LEVEL=%1 & shift & goto :getopt\n"
														"if \"%1\"==\"-3\" set _LEVEL=%1 & shift & goto :getopt\n"
														"if \"%1\"==\"-4\" set _LEVEL=%1 & shift & goto :getopt\n"
														"if \"%1\"==\"-5\" set _LEVEL=%1 & shift & goto :getopt\n"
														"if \"%1\"==\"-6\" set _LEVEL=%1 & shift & goto :getopt\n"
														...
														"if \"%1\"==\"-rmTq\" set _QUIET=1 & shift & goto :getopt\n"
														"\n")))
						 (cond ((string= "minizip" zip)
										(concat "\nREM ignore options\n\n"
														(let ((options nil))
															(dolist (x (append '("-r" "--filesync") ignore) options)
																(setq options
																			(concat options
						 ...
						 (cond ((string= "minizip" zip)
										(concat "\nREM minizip recursive call\n\n"
														...
														"if not \"%_file%\"==\"%_ZIP%\" (\n"
														"  if exist %_ZIP% (\n"
														"    minizip -a %_ZIP% %_LEVEL% %_file%\n"
														"  ) else (\n"
														"    minizip %_ZIP% %_LEVEL% %_file%\n"
														"  )\n"
														")\n"
														"goto :end\n"
														"\n:loop\n"
