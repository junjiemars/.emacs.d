...
(defsubst _mark_thing_ (begin end)
  "Mark thing at point."
  (goto-char begin)
  (set-mark (point))
  (goto-char end))
...
(defsubst _mark_quoted_symmetry@_ ()
...
(defsubst _mark_quoted_asymmetry@_ ()
  (let* ((p (point)) (l1 p) (r1 p)
         (lx (point-min)) (rx (point-max))
         (ls "\"'`([{<")
         (rs "\"'`)]}>"))
...
(defsubst _mark_sexp@_ (&optional n)
  (let* ((n1 (or n 1))
         (p (point))
         (fp (condition-case _
                 (save-excursion
...
(defsubst _mark_word@_ (&optional n)
  (let* ((n1 (or n 1))
