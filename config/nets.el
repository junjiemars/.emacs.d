  ...
  (defun *ping (&optional arg)
    ...
    (let* ((ping-program-options (if arg (split-string* arg " " t)
           ...
           (ipv6 (and (executable-find% "ping6")
                      (catch 'br
                        (dolist* (x ping-program-options)
                          (and (string= x "-6")
                               (throw 'br t))))))
           (ping-program-options
            (let ((xs nil))
              (dolist* (x ping-program-options (nreverse xs))
                (unless (or (null x) (and ipv6 (string= "-6" x)))
                  (setq xs (cons x xs))))))
  ...
  (defun *traceroute (&optional arg)
    ...
    (let* ((traceroute-program-options (if arg (split-string* arg " " t)
           ...
           (ipv6 (and (executable-find% "traceroute6")
                      (catch 'br
                        (dolist* (x traceroute-program-options)
                          (and (string= x "-6")
                               (throw 'br t))))))
           (traceroute-program-options
            (let ((xs nil))
              (dolist* (x traceroute-program-options (nreverse xs))
                (unless (or (null x)
                            (and ipv6 (string= "-6" x)))
                  (setq xs (cons x xs))))))
